...
function calculatePoints(rank, percent, minPercent) {
    // Rank outside top 150 gets 0 points
    if (rank < 1 || rank > 150) {
        return 0;
    }

    const basePoints = rankPoints[rank - 1];

    const multiplier = Math.pow(
        10,
        ((percent - minPercent) / (100 - minPercent)) - 1
    );
